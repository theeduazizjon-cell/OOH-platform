...
| DB                       | PostgreSQL 18 + PostGIS 3.6                                                                                                               | See 07                                                                                                                                  |                                                                                                     |
...
- **Upload**: `POST /files/uploads` → server creates `file_object(PENDING)` + presigned PUT (size/MIME-limited,
  ...
  magic bytes (not trusting the extension), **ClamAV scan**, strip location-sensitive EXIF from _derivatives only_ (originals
...
- **Tools for the assistant** (read-only, tenant- and permission-scoped as the _requesting user_):
...
| Integration (DB)     | Vitest + Testcontainers `postgis/postgis:18-3.6` | Repositories under real RLS; exclusion constraints (double booking, duplicate tariffs, overlapping asset terms); spatial queries (radius, bbox, point-in-area, fixtures with known distances); triggers (immutability of LOCKED lines, ACTIVE tariffs) |
...
├─ infrastructure/
│  ├─ docker/              docker-compose.yml (postgis, redis, mailpit; object storage + clamav arrive in M4)
│  └─ terraform/           (later)
├─ docs/                   this package + adr/ + runbooks
├─ .github/workflows/      ci.yml
├─ turbo.json  pnpm-workspace.yaml  package.json  .env.example
├─ tsconfig.base.json  eslint.config.mjs  .prettierrc.json   (shared tooling config lives at the root)
```

There is no `packages/config` (root-level configs suffice for one app + two packages) and no `packages/ui` until a second frontend needs shared components. There is no separate `worker` app because it shares
