...
- **Specialised attributes**: common columns on `ooh_asset`. Type-specific simple attributes (pole material,
  ...
  `asset_type.attribute_schema` at write time. Only *structural* differences (mounts/faces) get tables. This is
...
Polymorphic references (`subject_type`, `subject_id`) are allowed only for *attachments to anything*: task,
